...
function add_to_cart(){
    ...
    $product_query = $this->db->query("SELECT * FROM product_list WHERE id = $pid");
    ...
    if($save){
        ...
        $this->db->query("UPDATE product_list SET quantity = $new_stock WHERE id = $pid");

        return 1; // Success
    } else {
        return 4; // Error in adding to cart
    }
}